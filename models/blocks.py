...
def DilatedConv(filters, conv_param):
    ...
    return block(filters, k=3, s=1, order=['b', 'r', 'c'],
                 order_param=[None, None, conv_param])

"""atrous spatial pyramid pooling"""
def ASPP(filters, strides, conv_param, dilation_rate_list,
         image_level_pool_size):
    # pyramid part
    def func(x):
        pyramid_1x1 = block(filters, 1, strides, order=['c', 'b', 'r'],
        ...
        image_level_feature = block(filters, 1, order=['ap', 'c'],
                                    order_param=[ap_param, conv_param])
        ...
        return block(filters, 1, strides, order=['c'],
                     order_param=[conv_param])(branch_logit)
    return func
...
def MR_global_path(filters, conv_param):
    # Include a global path in the merge and run net using dilated conv
    return block(filters, k=3, s=1, order=['b', 'r', 'c'],
                 order_param=[None, None, conv_param])
...
def MR_local_pr(filters, conv_param):
    def MR_local_pr(x):
        x = block(filters // 2, 1, 1, order=['c', 'r'],
                  order_param=[conv_param, None])(x)
        x = block(filters // 2, 3, 1,
                  order=['b', 'r', 'c', 'b', 'r', 'c'],
                  order_param=[None, None, conv_param,
                               None, None, conv_param])(x)
        return  block(filters, 1, 1, order=['c', 'r'],
                      order_param=[conv_param, None])(x)
    return MR_local_pr
...
def MR_global_pr(filters, conv_param):
    def MR_global_pr(x):
        x = block(filters // 2, 1, 1, order=['c', 'r'],
                  order_param=[conv_param, None])(x)
        x = block(filters, k=3, s=1, order=['b', 'r', 'c'],
...
def MR_block(filters, conv_param_local, conv_param_global):
    """implementation of the merge-and-run block in:
...
def MR_block_split(filters, conv_param):
    def MR_block_split(x):
        x = block(filters, 1, 1, order=['b', 'r', 'c'],
                  order_param=[None, None, conv_param])(x)
        x_out, y_out = x, x
        return x_out, y_out
...
def resnext_branch(fin, fout, conv_param):
    def resnext_branch(x):
        x = block(fin, 1, 1, order=['c', 'r'],
                  order_param=[conv_param, None])(x)
        x = block(fin, 3, 1, order=['b', 'r', 'c'], 
                  order_param=[None, None, conv_param])(x)
        return block(fout, 1, 1, order=['c', 'r'],
                     order_param=[conv_param, None])(x)
...
def resnext_block(sp, fin, fout, conv_param):
    def resnext_block(x):
        input = block(fout, 1, 1, order=['c', 'r'],
                      order_param=[conv_param, None])(x)
        branch = [input]
        for i in range(sp):
...
def res_block(filters, conv_param, scale=0.1):
    def res_block(x):
        res = block(filters, 3, 1, order=['c', 'r', 'c'],
                    order_param=[conv_param, None, conv_param])(x)
...
def dense_conv(filters, conv_param):
    ...
    """
    def dense_conv(x):
        x = block(filters, 3, 1, order=['b', 'r', 'c'],
                  order_param=[None, None, conv_param])(x)
        return block(filters, 1, 1, order=['b', 'r', 'c'],
                     order_param=[None, None, conv_param])(x)
    return dense_conv
...
def transitionLayerPool(filters,conv_param):
        return lambda x: block(filters, 1,1,order=['b','r','c','ap'],
                               order_param=[None, None,
                                            conv_param,{'pool_size':2}])(x)
...
def attention_layer_1(filters_out, conv_param, filters=16, alpha=1):
    def func (x1,x2):
        x1 = block(filters, 1, 1, order=['c','b', 'r' ],
                   order_param=[conv_param,None, None])(x1)
        x2 = block(filters, 1, 1, order=['c','b', 'r'],
                   order_param=[conv_param,None, None])(x2)
        x = block(filters, 1, 1, order=['r', 'b', 'c','s'],
                  order_param=[ None, None,conv_param,None])(x1+x2)
        return block(filters_out, 1, 1, order=['c','b', 'r'],
                     order_param=[conv_param,None, None])(x*alpha*x1)
    return func
"""
=== end blocks 
"""
