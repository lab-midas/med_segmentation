...
def plot_img(img, img_gamma, gamma, slice, slice_cut, name):

    img_slices= get_slices(img, slice_cut)
    img_gamma_slices = get_slices(img_gamma, slice_cut)
    slice=slice
    fig, ax = plt.subplots(nrows=1, ncols=2)
    ax[0].imshow(img_slices[slice])
    ax[0].set_title(slice_cut + " normal, slice: " + str(slice))
    ax[1].imshow(img_gamma_slices[slice])
    ax[1].set_title(name + ' ' + str(gamma))
    plt.show()
