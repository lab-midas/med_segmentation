...
def get_slice_most_1(mask, slice_cut='a', patch=96):
    axis = 2
    if slice_cut =='a':
        axis = 2
    if slice_cut =='s':
    ...
    label_1 = np.any(mask[...]==1, axis=axis)
    label_1_id = np.argwhere(label_1==True)
    if label_1_id.size:
        rnd = np.random.randint(0, label_1_id.shape[0])
        idx = label_1_id[rnd]
        # Sample additional index along the third axis(=2).
        # Voxel value should be equal to the class value.
        valid_idx = mask[idx[0], idx[1], :]
...
def plot_combine(img, mask, index=0, slice_cut='a', patch=96):
    #img_slices = get_slices(img, slice_cut, patch=patch)
    #mask_slices = get_slices(mask, slice_cut, patch=patch)
    slice_idx = get_slice_most_1(mask[..., 1], slice_cut, patch=patch)
    if slice_idx is not None:
        img_slice = img[..., slice_idx, 1]
        mask_slice = mask[..., slice_idx, 1]
        num_category = len(np.unique(mask))

        plt.imshow(img_slice)
        plt.imshow(mask_slice, alpha=0.7)
        plt.show()
