    ...
    if history.history != {} and history.history is not None:
        if mode == 'train_val':
            ...
            with open(path_pickle +config['model']+'_'+ dataset + '_k_fold_' + str(k_fold_index) + '.pickle', 'wb') as fp:
                ...
                print('Successfully save the ' + mode + ' loss and metrics of  ' + dataset + '.')
    else:
        print('Warning! No training / Validation loss or metric data! ')


def color_set(num_categories, custom_colormap=None, random_colormap=False, custom_max_value=255, custom_start=0,
              random_scale=1):
    """
    ...
    :param custom_colormap: type list of list of 3 float: costume colormap
    :param random_colormap: type bool: True if random colors are chosen
    :param custom_max_value: type float: max value if default colormap is chosen
    :param custom_start: type int: start point of extraction from default colormap
    ...
    if custom_colormap is None and random_colormap is False:
        ...
        default_colormap = (np.array([[0, 0, 0.8], [0, 0.8, 0], [0.8, 0, 0], [0.1, 0, 0.8], [0.3, 0.5, 0.1],
                            ...
                            * custom_max_value)
        assert (num_categories + custom_start <= len(default_colormap))
        return default_colormap[custom_start:num_categories + custom_start]

    elif custom_colormap is not None:
        return custom_colormap[:num_categories]
    elif random_colormap:
        colormap_set = [np.array([i, j, k]) * random_scale for k in np.arange(0, 255, 1) for j in np.arange(0, 255, 1)
                        for i in
                        np.arange(0, 255, 1)]
        color_map_index = np.random.randint(len(colormap_set), size=num_categories)
        color_map = np.array([colormap_set[i] for i in color_map_index])
        return color_map
    else:
        return None
...
def plot_mosaic(config, mask, slice_dim=2, colormap=None, vspace=2, hspace=2, col=5, rotate_k=None, flip_axis=None,
    ...
    colormap = color_set(num_categories=num_category, custom_colormap=colormap)
    ...
    path_figures = dir_figures + '/' + '_'.join(['mosaic', image_type, config['model'], dataset, name_ID, 'slice_dim',
                                                 str(slice_dim)]) + '.png'
    figure.save(path_figures)


def plot_area_ratio(config, list_images_series, slice_dim=2, merge_channel_plot=False, yscale='linear',
                    plot_label_series=None, dataset='data', figsize=None,
                    plot_mean_only=False, client_save_rootdir=None):
    """ Plot the mean and standard variation of the area ratio depending of slice of the dataset.
        ...
        :param figsize: tbd
        :param plot_mean_only: tbd
        :param client_save_rootdir: Specify custom directory outside of config YAML
    """
    ...
    if merge_channel_plot:  # Merge all series and all channels in one figure.
        ...
        line_style = ['-', '--', '-.', ':']
        for i in range(len(list_images_series)):
            colorset = color_set(num_categories=channel, custom_max_value=1, custom_start=i * channel)
            for ch in range(channel):
        ...
        for ch in range(channel):
            ...
            colorset = color_set(num_categories=len_series, custom_max_value=1, custom_start=ch * len_series)
            line_style = ['-', '--', '-.', ':']
            for i in range(len(list_images_series)):
                # Plot standard variation gap
                if not plot_mean_only:
            ...
            plt.legend(loc="upper right")
            # Save figure
            path_dir = os.path.join(config['result_rootdir'], config['model'], 'figures', 'plot_area_ratio', dataset)
            if client_save_rootdir is not None:
                path_dir = os.path.join(client_save_rootdir, path_dir)
            if not os.path.exists(path_dir): os.makedirs(path_dir)
            path_figures = os.path.join(path_dir, '_'.join(['area', 'ratio', 'merged', 'plot',
                                                            'channel', str(ch)]) + '.png')
            plt.savefig(path_figures)
            plt.close(ch)
...
def plot_by_slice(config, masks_list, slice_, channel_only=None, slice_dim=2, space=1, title_list=None,
    ...
    :param channel_only: type int: channel index of displaying image
    :param slice_dim: type int: dimension index of slicing.
            For 3D, 0 for Sagittal plain, 1 for Coronal plane, 2 for Axial plane.
    ...
    for image_index in range(0, num_masks):
        ...
        colormap = color_set(num_categories=mask_categories + 1, custom_colormap=colormap)  # use default colorset
        # Color the image
    ...
    # Save figure
    dir_figures = os.path.join(config['result_rootdir'], config['model'], 'figures', 'plot_by_slice', dataset, name_id)
    if client_save_rootdir is not None:
        dir_figures = client_save_rootdir + '/' + dir_figures
    if not os.path.exists(dir_figures): os.makedirs(dir_figures)
    path_figures = os.path.join(dir_figures, '_'.join(['profile', config['model'], name_id, 'plane',
                                                       str(slice_dim), 'slice', str(slice_)]) + '.png')
    figure.save(path_figures)
